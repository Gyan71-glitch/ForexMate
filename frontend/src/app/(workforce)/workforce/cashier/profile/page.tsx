...
export default function CashierProfilePage() {
  ...
  const handleChangePassword = () => router.push('/workforce/change-password');

  return (
    <div style={{ paddingBottom: 80, background: '#f8fafc', minHeight: '100dvh' }}>
      {/* Profile Hero */}
      <div style={{ background: 'linear-gradient(135deg, #1e1b4b, #4338CA)', padding: '32px 20px 28px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 12, margin: '0 0 4px', fontWeight: 600 }}>{employee?.employeeCode}</p>
        <span style={{ background: 'rgba(255,255,255,0.15)', color: 'white', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.2)' }}>Branch Cashier</span>
      </div>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div style={cardStyle}>
          ...
          <ProfileRow icon="🏢" label="Branch" value={employee?.branchName || 'N/A'} />
          <ProfileRow icon="📱" label="Phone" value={employee?.phone || 'N/A'} />
          <ProfileRow icon="📧" label="Email" value={employee?.email || 'N/A'} />
          <ProfileRow icon="🪪" label="Employee ID" value={employee?.employeeCode || 'N/A'} />
        </div>

        {/* Actions */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 4 }}>
          <button onClick={handleChangePassword} style={{ ...actionBtnStyle, background: '#eef2ff', color: '#4338CA' }}>
            🔐 Change Password
          </button>
          <button
            onClick={() => {
      ...
      <BottomNav tabs={CASHIER_TABS} active="profile" theme="indigo" />
    </div>
  );
}
...
const cardStyle: React.CSSProperties = { background: 'white', borderRadius: 16, padding: 16, border: '1px solid #f1f5f9', marginBottom: 12 };
const sectionTitle: React.CSSProperties = { fontSize: 11, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 8px' };
const actionBtnStyle: React.CSSProperties = { width: '100%', padding: '14px 16px', borderRadius: 14, border: 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' as const };
