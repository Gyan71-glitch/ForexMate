...
let PermissionsGuard = class PermissionsGuard {
    ...
    async canActivate(context) {
        ...
        if (!user || !user.roleId) {
            throw new common_1.ForbiddenException('Access denied. No active role associated.');
        }
        ...
        const userPermissions = roleWithPermissions.permissions.map((rp) => rp.permission.action);
        const hasPermission = requiredPermissions.every((permission) => userPermissions.includes(permission));
        if (!hasPermission) {
            throw new common_1.ForbiddenException('You do not have the required permissions to perform this action.');
        }
        return true;
    }
};
