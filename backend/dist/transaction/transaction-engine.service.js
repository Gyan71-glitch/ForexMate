...
var TransactionEngineService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.TransactionEngineService = void 0;
...
let TransactionEngineService = TransactionEngineService_1 = class TransactionEngineService {
    prisma;
    quotesService;
    eventBus;
    logger = new common_1.Logger(TransactionEngineService_1.name);
    constructor(prisma, quotesService, eventBus) {
        this.prisma = prisma;
    ...
    async createSession(userId) {
        const session = await this.prisma.transactionSession.create({
            data: {
                userId: userId || null,
                status: client_1.SessionStatus.CREATED,
                draftState: {},
            },
        });
    ...
    async updateDraftState(sessionId, draftState, requestingUserId) {
        const session = await this.prisma.transactionSession.findUnique({ where: { id: sessionId } });
        if (!session)
            throw new common_1.NotFoundException('Session not found');
        if (session.status === client_1.SessionStatus.CONVERTED)
    ...
    async getWorkflowNextStep(sessionId, requestingUserId) {
        ...
        if (state.product && state.currency && state.amount) {
            ...
            if (!session.userId) {
                currentState = client_1.SessionStatus.WAITING_LOGIN;
                allowedActions = ['LOGIN_OR_REGISTER'];
            }
        }
        if (session.status === client_1.SessionStatus.QUOTE_LOCKED) {
            progress = 50;
        ...
        if (session.status === client_1.SessionStatus.WAITING_PAYMENT) {
            progress = 90;
    ...
    async generateAndLockQuote(sessionId, dto) {
        const session = await this.prisma.transactionSession.findUnique({ where: { id: sessionId } });
        if (!session)
            throw new common_1.NotFoundException('Session not found');
        if (session.status === client_1.SessionStatus.CONVERTED)
            throw new common_1.BadRequestException('Session is already converted to an order');
        if (!session.userId)
            throw new common_1.BadRequestException('User must be authenticated to lock a quote');
        ...
        await this.prisma.transactionSession.update({
            where: { id: sessionId },
            data: { status: client_1.SessionStatus.QUOTE_LOCKED }
        });
        return quoteData;
    }
    async checkout(sessionId, idempotencyKey) {
        ...
        if (session.idempotencyKey && session.idempotencyKey !== idempotencyKey) {
            throw new common_1.BadRequestException('Invalid idempotency key provided for this session');
        }
        const activeQuote = session.quotes[0];
        if (!activeQuote)
            throw new common_1.BadRequestException('No active quote found for this session');
        ...
        if (activeQuote.expiresAt < currentDate) {
            ...
            await this.prisma.transactionSession.update({ where: { id: sessionId }, data: { status: client_1.SessionStatus.EXPIRED } });
            throw new common_1.BadRequestException('Quote has expired. Please generate a new quote.');
        }
        if (!session.userId)
            throw new common_1.BadRequestException('User must be authenticated to checkout');
        const profile = await this.prisma.customerProfile.findUnique({
            where: { userId: session.userId },
            include: { user: true }
        });
        if (!profile)
            throw new common_1.BadRequestException('Customer profile incomplete');
        const draftState = session.draftState || {};
        let branchId = draftState.branchId;
        if (!branchId) {
            const defaultBranch = await this.prisma.branch.findFirst();
            if (!defaultBranch)
                throw new common_1.BadRequestException('System configuration error: No branches available');
        ...
        if (!productCode)
            throw new common_1.BadRequestException('Product code not found in draft state');
        const forexProduct = await this.prisma.forexProduct.findUnique({ where: { code: productCode } });
        if (!forexProduct)
            throw new common_1.BadRequestException(`Invalid product code: ${productCode}`);
        ...
        const order = await this.prisma.$transaction(async (tx) => {
            const initialStatus = (productType === 'CASH_SELL' || isRemittance) ? client_1.OrderStatus.PENDING : client_1.OrderStatus.PAYMENT_PENDING;
            ...
            await tx.transactionSession.update({
                where: { id: session.id },
                data: { status: client_1.SessionStatus.CONVERTED, idempotencyKey }
            });
            ...
            if (productType === 'CASH_SELL') {
                ...
                const opsStaff = branchStaff.filter((s) => s.user.roleRef?.name === 'BRANCH_OPERATIONS_STAFF' ||
...
exports.TransactionEngineService = TransactionEngineService;
exports.TransactionEngineService = TransactionEngineService = TransactionEngineService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        quotes_service_1.QuotesService,
        domain_event_bus_service_1.DomainEventBus])
], TransactionEngineService);
//# sourceMappingURL=transaction-engine.service.js.map