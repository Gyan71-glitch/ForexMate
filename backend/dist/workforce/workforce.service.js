...
let WorkforceService = WorkforceService_1 = class WorkforceService {
    prisma;
    jwtService;
    eventBus;
    logger = new common_1.Logger(WorkforceService_1.name);
    constructor(prisma, jwtService, eventBus) {
        this.prisma = prisma;
    ...
    async getSystemUserId() {
        ...
        if (!admin) {
            ...
            if (!anyUser)
                throw new Error('No system user found for status history.');
            return anyUser.id;
        }
        return admin.id;
    }
    ...
    async getAssignedOrders(employeeId, role) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
    ...
    async getOrderDetail(orderId, employeeId, role) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
    ...
    async getHistory(employeeId, role) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        if (role === 'BRANCH_CASHIER') {
    ...
    async sendCustomerOtp(orderId, dto, employeeId, role) {
    ...
    async verifyCustomerOtp(orderId, dto, employeeId, role) {
        await this.getOrderDetail(orderId, employeeId, role);
    ...
    async completePickup(orderId, employeeId) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const cashier = await this.prisma.cashier.findUnique({ where: { employeeCode: employee.employeeCode } });
        ...
        if (order.cashierId !== cashier.id)
            throw new common_1.UnauthorizedException('Not assigned to this order.');
    ...
    async completeCashSell(orderId, employeeId) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const cashier = await this.prisma.cashier.findUnique({ where: { employeeCode: employee.employeeCode } });
    ...
    async reachedCustomer(orderId, employeeId) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const dp = await this.prisma.deliveryPartner.findUnique({ where: { employeeCode: employee.employeeCode } });
    ...
    async completeDelivery(orderId, dto, employeeId) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const dp = await this.prisma.deliveryPartner.findUnique({ where: { employeeCode: employee.employeeCode } });
    ...
    async getCityInventory(employeeId) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
    ...
    async reassignBranch(orderId, employeeId, dto) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
        ...
        if (currentBranch && currentBranch.branchCity !== targetBranch.branchCity) {
            throw new common_1.BadRequestException(`Reassignment restricted to branches within the same city (${currentBranch.branchCity}). Destination branch is in ${targetBranch.branchCity}.`);
        }
        const systemUserId = await this.getSystemUserId();
    ...
    async completePickupByManager(orderId, employeeId, dto) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            if (order.items && order.items.length > 0) {
                for (const item of order.items) {
                    const currencyCode = item.currency?.code || item.currencyCode || 'USD';
    ...
    async assignDeliveryPartner(orderId, employeeId, dto) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
    ...
    async allocateCash(orderId, employeeId, items) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        ...
        if (Math.abs(totalAllocated - Number(orderItem.amount)) > 0.001) {
            throw new common_1.BadRequestException(`Allocated total (${totalAllocated}) does not match requested amount (${orderItem.amount})`);
        }
        return this.prisma.$transaction(async (tx) => {
    ...
    async getManagerDashboard(employeeId) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
        ...
        const [todayOrders, pendingPickups, pendingDeliveries, completedToday, pendingCashAllocation, branchInventory, auditLogs,] = await Promise.all([
    ...
    async getDeliveryPartners(employeeId) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
    ...
    async getManagerReports(employeeId) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
    ...
    async getManagerTimeline(employeeId) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
    ...
    async holdOrder(orderId, employeeId, reason) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
    ...
    async escalateOrder(orderId, employeeId, reason) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const order = await this.prisma.order.update({
    ...
    async reportFraud(orderId, employeeId, reason) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const order = await this.prisma.order.update({
    ...
    async cancelPickup(orderId, employeeId, reason) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee)
            throw new common_1.NotFoundException('Employee not found.');
        const order = await this.prisma.order.update({
    ...
    async receiveBranchInventory(employeeId, dto) {
        const employee = await this.prisma.employee.findUnique({
            where: { id: employeeId },
            include: { branch: true },
        });
        ...
        if (!employee.branchId)
            throw new common_1.BadRequestException('Employee has no assigned branch.');
        if (dto.amount <= 0)
            throw new common_1.BadRequestException('Receipt amount must be greater than zero.');
        if (!dto.referenceNumber)
            throw new common_1.BadRequestException('Reference number is mandatory for inventory receipt.');
        ...
        if ((dto.sourceType === 'HQ_TREASURY_TRANSFER' || dto.sourceType === 'COMMERCIAL_BANK_COLLECTION') && !dto.treasurySlipPhotoUrl) {
            throw new common_1.BadRequestException(`Treasury Slip photo evidence is mandatory for ${dto.sourceType.replace(/_/g, ' ')}.`);
        }
        if ((dto.sourceType === 'CUSTOMER_CASH_SELL' || dto.sourceType === 'INTER_BRANCH_TRANSFER') && !dto.currencyBundlePhotoUrl) {
            throw new common_1.BadRequestException(`Currency Bundle photo evidence is mandatory for ${dto.sourceType.replace(/_/g, ' ')}.`);
        }
        const branchId = employee.branchId;
        const result = await this.prisma.$transaction(async (tx) => {
...
exports.WorkforceService = WorkforceService = WorkforceService_1 = __decorate([
