...
let AdminService = class AdminService {
  ...
  async getExecutiveMetrics() {
    ...
    const branchHealth = branches.map((b) => {
      const totalStock = b.branchInventory.reduce(
        (acc, inv) => acc + Number(inv.availableAmount || 0),
        0,
      );
      return {
        id: b.id,
  ...
  async getAllOrders(user) {
    const orders = await this.prisma.order.findMany({
      ...
      include: {
        ...
        branch: {
          select: { branchName: true, branchCity: true, branchCode: true },
        },
  ...
  async createBranch(dto, userId) {
    ...
    if (!company) {
      throw new common_1.BadRequestException(
        'Company profile not initialized.',
      );
    }
    const branch = await this.prisma.branch.create({
      data: {
        ...
        vaultCapacity: dto.vaultCapacity ? dto.vaultCapacity : 10000000.0,
      },
    });
  ...
  async assignBranchManager(branchId, employeeId, adminUserId) {
    ...
    if (employee.role !== 'BRANCH_MANAGER') {
      throw new common_1.BadRequestException(
        `Employee '${employee.name}' does not have the BRANCH_MANAGER role.`,
      );
    }
