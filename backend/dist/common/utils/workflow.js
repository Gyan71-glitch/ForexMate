...
exports.activateBranchWorkflow = activateBranchWorkflow;
function mapOrderStatus(order) {
    if (order.productType === 'REMITTANCE') {
        ...
        if (order.status === 'PENDING')
            return 'PENDING_KYC';
        if (order.status === 'KYC_SUBMITTED' || order.status === 'AML_PENDING' || order.status === 'LRS_PENDING')
            return 'COMPLIANCE_REVIEW';
        if (order.status === 'READY_TO_FORWARD')
            return 'PROCESSING';
        if (order.status === 'FORWARDED_TO_PARTNER' || order.status === 'PARTNER_PROCESSING')
            return 'TRANSFER_PROCESSING';
        if (order.status === 'TRANSFER_COMPLETED' || order.status === 'COMPLETED')
            return 'COMPLETED';
        return order.status;
    }
    ...
    if (order.status === 'PAYMENT_COMPLETED') {
        ...
        const kycTask = tasks.find((t) => t.taskType === 'KYC_REVIEW');
        const inventoryTask = tasks.find((t) => t.taskType === 'INVENTORY_PREP');
        ...
        if (inventoryTask && inventoryTask.status === 'COMPLETED') {
            if (order.deliveryMethod === 'HOME_DELIVERY') {
                const handoverTask = tasks.find((t) => t.taskType === 'HANDOVER');
...
async function activateBranchWorkflow(orderId, tx) {
